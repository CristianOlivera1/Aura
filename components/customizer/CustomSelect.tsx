...
export function CustomSelect({ value, options, onChange, className = "" }: Props) {
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center justify-between w-full bg-white/5 border border-white/10 hover:border-white/25 text-white text-[11px] rounded-md px-2.5 py-1.5 outline-none transition-colors"
      >
